...
- [ ] Open **Chat Studio** from the ReplyPen project navigation, select a real tenant/principal when the project requires one, and send one preview message.
...
declared principal kinds, and exact origins; do not ask them to choose those values again. The prose
above it separates what is already live in ReplyPen from the backend and browser work you still own.
On a tenant Studio route, project-wide branding, presentation, enablement, and origins are read-only;
the tenant admin can change only that tenant's chat depth.
